...
export default function DashboardPage() {
  ...
  personas.forEach((p,i) => {
    ...
    if (p.status==='촬영' && !p.confirmed) actions.push({l:`페르소나 ${i+1}`,a:'인플루언서 확정 필요',c:'#E53E3E'})
  })
      ...
      <div className="p-7">
        <div className="grid grid-cols-5 gap-2.5 mb-5">
          {[
            ...
            {val:personas.filter(p=>p.status==='촬영').length,label:'촬영 진행 중',c:'#F59E0B',bc:'chip-amber'},
            ...
            <div className="card-body p-0">
              ...
              {personas.map(p=>(
                <div key={p.id} className="grid border-b border-gray-50 hover:bg-gray-50" style={{gridTemplateColumns:'1fr 70px 70px 50px'}}>
                  ...
                  <div className="px-3 py-2 text-xs font-bold font-mono" style={{color:p.grade?({S:'#7C3AED',A:'#1D9E75',B:'#F59E0B',C:'#E53E3E'} as Record<string,string>)[p.grade]:'#A0AEC0'}}>{p.grade||'—'}</div>
                </div>
              ))}
            </div>
          </div>
