...
export default function ProductsPage() {
  ...
  const [seedStatusMap, setSeedStatusMap] = useState<Record<string, boolean>>({})
  ...
  async function loadAll() {
    ...
    await Promise.all(
      productsData.map(async (prod: Product) => {
        if (prod.id) {
          try {
            const res = await fetch(`/api/product-seeds/${prod.id}`)
            const seed = await res.json()
            statusMap[prod.id] = seed && seed.target_description ? true : false
          } catch {
      ...
      <div className="p-7 space-y-5">
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              카테고리 관리 <span className="text-gray-400 text-xs ml-1">({categories.length})</span>
            </span>
          </div>
          <div className="card-body">
            <div className="flex flex-wrap gap-2 mb-3">
              {categories.map((c) => (
                <span
                  key={c.id}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-full text-xs"
                >
                  {c.name}
                  <button
        ...
        <div className="card">
          ...
          {loading ? (
            ...
            <div>
              <div
                className="bg-[#0D1117] grid text-[10px] text-gray-400 font-semibold uppercase tracking-wide"
                style={{ gridTemplateColumns: '60px 1fr 100px 100px 110px 110px 180px' }}
              >
                {['이미지', '상품명', '카테고리', '브랜드', '가격', '시드 데이터', '관리'].map((h) => (
              ...
              {filtered.map((p) => (
                <div
                  key={p.id}
                  className="grid border-b border-gray-50 hover:bg-gray-50 transition-colors items-center"
                  style={{ gridTemplateColumns: '60px 1fr 100px 100px 110px 110px 180px' }}
                >
                  ...
                  <div className="px-3 py-3">
                    {p.id && seedStatusMap[p.id] ? (
                      <span className="bg-green-50 text-green-800 text-[10px] px-2 py-1 rounded-full font-medium">
                        ✓ 세팅됨
                      </span>
                    ) : (
                      <span className="bg-amber-50 text-amber-800 text-[10px] px-2 py-1 rounded-full font-medium">
                        ⚠ 미세팅
                      </span>
                    )}
                  </div>
                  <div className="px-3 py-3 flex gap-1">
                    <Link
                      href={`/products/${p.id}/seed`}
                      className="bg-[#1D9E75] text-white text-[10px] px-2 py-1 rounded hover:bg-[#0F6E56]"
                    >
                      🌱 시드
                    </Link>
                    <Link href={`/products/add?id=${p.id}`} className="btn-sm">
                      수정
                    </Link>
                    <button
                      onClick={() => p.id && deleteProduct(p.id)}
                      className="btn-sm text-red-500 hover:border-red-400"
                    >
                      삭제
                    </button>
                  </div>
