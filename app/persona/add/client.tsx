...
export default function PersonaAddClient() {
  const router = useRouter()
  const params = useSearchParams()
  const editId = params.get('id')
  const linkedCampaignId = params.get('campaign')
  ...
  useEffect(() => {
    if (linkedCampaignId && !editId) {
      setForm(f => ({...f, campaign_id: linkedCampaignId}))
    }
    if (!editId) return
  ...
  async function save() {
    ...
    else if (linkedCampaignId) router.push(`/campaigns/${linkedCampaignId}`)
    else router.push('/')
  }
